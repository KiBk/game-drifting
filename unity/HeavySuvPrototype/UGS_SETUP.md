...
4. In the Unity Dashboard, enable **Authentication** and **Relay** for the linked environment.
...
Each browser opened without a `join` query creates a fresh Relay allocation supporting eight active drivers. The host shares the displayed `?join=<relay-code>` link, and invited browsers join that allocation directly. Relay client-host networking uses secure WebSockets in `europe-north1` and does not depend on Lobby event subscriptions. Bump the Netcode protocol version whenever a release makes network-incompatible prefab or behavior changes. If the host leaves, the Relay code expires and another player must create a fresh room.
